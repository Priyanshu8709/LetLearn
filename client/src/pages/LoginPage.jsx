...
export default function LoginPage() {
  ...
  const onSubmit = async ({ email, password }) => {
    try {
      ...
      const dashboards = { admin: '/admin', instructor: '/instructor/dashboard', student: '/dashboard' }
      navigate(from ?? dashboards[res.user?.accountType] ?? '/dashboard', { replace: true })
    } catch (err) {
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-surface-1 to-surface-0">
        ...
        <div className="relative z-10 flex flex-col justify-center px-16">
          ...
          <p className="text-white/40 text-lg leading-relaxed max-w-sm">
            Pick up right where you left off. Your courses, progress, and certifications are waiting.
          </p>
          <div className="mt-14 space-y-4">
            {[
              { label: '50,000+ Active Learners', sub: 'Join a thriving community' },
              { label: '1,200+ Courses', sub: 'From beginner to expert' },
              { label: 'Track Your Progress', sub: 'Know exactly where you stand' },
            ].map((item) => (
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="flex justify-end">
              <Link to="/reset-password" className="text-sm text-brand-400 hover:text-brand-300 transition-colors">
                Forgot password?
              </Link>
            </div>
